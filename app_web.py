...
def afficher_logo(largeur=None):
    if os.path.exists(NOM_LOGO):
        st.image(NOM_LOGO, width=largeur if largeur else None, use_container_width=not largeur)
    else:
        st.write("### PAPREC")
...
def enregistrer_donnees(mode, header, dict_poids):
    ...
    try:
        ...
        ligne = ([header['flux'], header['date'], header['equipe'], header['lieu']] + poids_finaux) if mode == "CARACT SORTANT" else ([header['flux'], header['date']] + poids_finaux)
...
if 'last_flux' not in st.session_state: st.session_state.last_flux = 0
if 'last_equipe' not in st.session_state: st.session_state.last_equipe = 0
if 'last_lieu' not in st.session_state: st.session_state.last_lieu = 0
...
# --- ECRAN DE SAISIE ---
    ...
    with st.container(border=True):
        ...
        # Utilisation des index mémorisés pour aller plus vite
        flux_sel = col2.selectbox(lbl, lst, index=st.session_state.last_flux, key="sel_flux")
        equipe_sel = col3.selectbox("Équipe:", LISTE_EQUIPES, index=st.session_state.last_equipe, key="sel_equipe")
        lieu_sel = col4.selectbox("Lieu:", LISTE_LIEUX, index=st.session_state.last_lieu, key="sel_lieu")
            ...
            for j in range(2):
                if i + j < len(items):
                    ...
                    with cols[j].container(border=True):
                        ...
                        # --- SYSTÈME PHOTO SIMPLIFIÉ ---
                        with c_photo.popover("📸 Photo"):
                            if matiere in st.session_state.photos_temp:
                                st.image(st.session_state.photos_temp[matiere], width=180)
                                if st.button("🔄 Reprendre", key=f"re_{matiere}"):
                                    del st.session_state.photos_temp[matiere]
                                    st.rerun()
                            else:
                                img = st.camera_input(f"Capture {matiere}", key=f"cam_{matiere}")
                                if img:
                                    st.session_state.photos_temp[matiere] = img
                                    st.rerun()

    st.markdown("---")
    if st.button("💾 ENREGISTRER ET NOUVELLE SAISIE", type="primary", use_container_width=True):
        # Mémorisation des index pour la prochaine fois avant de reset
        st.session_state.last_flux = lst.index(flux_sel)
        st.session_state.last_equipe = LISTE_EQUIPES.index(equipe_sel)
        st.session_state.last_lieu = LISTE_LIEUX.index(lieu_sel)
        
        date_folder = date_saisie.replace("/", "-")
        path_complet = os.path.join("PHOTOS_CARACT", "ENTRANT" if st.session_state.mode == "CARACT ENTRANT" else "SORTANT", f"{flux_sel}_{date_folder}")
        ...
        try:
            ...
            if enregistrer_donnees(st.session_state.mode, h, dict_entrees):
                ...
                # Reset des données de saisie mais conservation du mode et des sélections
                st.session_state.photos_temp.clear()
                for key in list(st.session_state.keys()):
                    if key.startswith("p_") or key.startswith("cam_"):
                        del st.session_state[key]
                
                # On retourne à l'accueil pour changer de type de caract si besoin
                st.session_state.mode = None
                st.rerun()
        except Exception as e:
            st.error(f"❌ Erreur : {e}")
